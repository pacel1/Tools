...
export default function Base64EncoderTool() {
  ...
  const result = useMemo(() => runBase64Encoder({ text }), [text]);
  return (
    <div className="space-y-5">
      <div>
        <label className="mb-2 block text-sm text-white/70" htmlFor="base64-encoder-text">{t.input}</label>
        <textarea id="base64-encoder-text" className="min-h-32 w-full rounded-2xl border border-white/10 bg-slate-950/90 px-4 py-3 outline-none transition focus:border-cyan-300/50" value={text} onChange={(event) => setText(event.target.value)} />
      </div>
